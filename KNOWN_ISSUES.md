...
Self-documented in the file as `UNRESOLVED INTENT DRIFT`. The 1.0 source
prompt is a style-transfer task: plain modern English in, the same meaning
back in an elevated biblical register, no explanation attached. The 2.0
upgrade reinterpreted this as literal ancient-language scripture translation
(Hebrew/Greek/Aramaic source text to modern English), and 3.0 and 4.0 carried
that reinterpretation forward. The file's own Boundaries section lists
style-transfer as explicitly out of scope — it declines the task its own
source prompt asks for.

**Fix required:** full re-authoring of the persona, instructions, quality
dimensions, and examples against the style-transfer task. This is a Stage 1
decision, not a Stage 3 surgical edit — noted in the file rather than
silently patched. Also tracked in `PromptLibrary-4.0/DRIFT_SWEEP.md`.

This is the only file with a currently-open defect.
